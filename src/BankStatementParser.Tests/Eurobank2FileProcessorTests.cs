...
{
    public class Eurobank2FileProcessorTests
    {
        private Eurobank2FileProcessor _fileProcessor;
        private TransactionSerializer _transactionSerializer;

        [SetUp]
        public void Setup()
        {
            _fileProcessor = new Eurobank2FileProcessor();
            _transactionSerializer = new TransactionSerializer();
        }

        private static string[] GetPdfFiles() =>
            Directory.GetFiles("./test-data/eurobank2", "*.pdf");
        ...
        {
            var statements = _fileProcessor.Process("test-data/eurobank2/1.pdf");
        ...
        {
            var statement = _fileProcessor.Process("test-data/eurobank2/1.pdf")
                .ShouldHaveSingleItem();

            statement.FromDate.ShouldBe(new DateTime(2023, 09, 01));
            statement.ToDate.ShouldBe(new DateTime(2023, 11, 30));
        }
    }
}